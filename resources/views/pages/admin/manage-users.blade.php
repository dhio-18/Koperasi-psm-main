  ...
  <div x-data="userPage()" class="min-h-screen px-4 py-6"><!-- px-6→4, py-10→6 -->
        ...
        <table class="w-full">
          <thead class="bg-green-50 sticky top-0 z-10">
            <tr class="text-gray-900 text-xs md:text-sm">
              <th class="px-4 md:px-6 py-3 text-left font-medium uppercase tracking-wider">#</th><!-- py-4→3 -->
          ...
          <tbody class="bg-white divide-y divide-gray-200">
            <template x-for="(user, index) in filteredUsers()" :key="user.id">
              <tr class="hover:bg-gray-50 transition-colors">
                <td class="px-4 md:px-6 py-3 whitespace-nowrap text-gray-900 font-medium" x-text="index + 1"></td><!-- py-4→3 -->
                ...
                <td class="px-4 md:px-6 py-3 whitespace-nowrap text-right">
                  <div class="flex justify-center gap-2"><!-- space-x-3→gap-2 -->
                    ...
                    <button @click="confirmDelete(user.id)" class="text-red-600 hover:text-red-700 transition-colors"
                      title="Hapus">
                      <svg class="w-5 h-5 md:w-6 md:h-6" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                        <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
    ...
    <div x-show="showConfirmModal" class="fixed inset-0 z-50 overflow-y-auto" x-cloak>
      <div class="flex items-center justify-center min-h-screen px-4 text-center sm:block sm:p-0">
        <div x-show="showConfirmModal" x-transition:enter="ease-out duration-200" x-transition:enter-start="opacity-0"
          x-transition:enter-end="opacity-100" x-transition:leave="ease-in duration-150"
          x-transition:leave-start="opacity-100" x-transition:leave-end="opacity-0"
          class="fixed inset-0 transition-opacity bg-gray-500/70" @click="showConfirmModal = false"></div>
        ...
        <div x-show="showConfirmModal" x-transition:enter="ease-out duration-200"
          ...
          class="inline-block w-full max-w-sm p-5 my-8 overflow-hidden text-left align-middle transition-all transform bg-white shadow-xl rounded-lg"><!-- max-w-md→sm, p-6→5 -->
          <div class="flex items-center justify-center w-10 h-10 mx-auto mb-3 bg-red-100 rounded-full text-red-500">
          ...
          <h3 class="text-base font-semibold text-gray-900 text-center mb-1.5">Hapus Akun Admin</h3><!-- text-lg→base -->
          <p class="text-sm text-gray-600 text-center mb-4">Apakah Anda yakin untuk menghapus admin ini? Klik batal jika
    ...
    <div x-show="showModal" x-cloak class="fixed inset-0 bg-black/40 flex items-center justify-center z-50">
      <div @click.away="closeModal()" class="bg-white w-full max-w-sm rounded-xl shadow-lg p-5"><!-- max-w-md→sm, p-6→5 -->
        ...
        <form :action="isEdit
                              ...
                              : baseUrl + 'superadmin/manage-users/'" method="POST">
          @csrf
          ...
          <div class="flex justify-end gap-2 pt-3 border-t"><!-- pt-4→3 -->
            ...
            <button type="submit" class="px-3.5 py-1.5 bg-green-600 text-white rounded-lg hover:bg-green-700 text-sm"

    ...
    <div class="p-6 space-y-6 bg-gray-50 min-h-screen" x-data="reportCharts()" x-init="init()">
        ...
        <div class="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4">
            <div>
                ...
                <p class="text-gray-600">Laporan {{ $periodLabel }} - {{ $startDate->format('d M Y') }} s/d {{ $endDate->format('d M Y') }}</p>
            </div>
        </div>
        ...
        <div class="bg-white p-6 rounded-xl shadow-md border border-gray-100">
            ...
            <form method="GET" action="{{ route('superadmin.financial-report') }}" class="grid grid-cols-1 md:grid-cols-3 gap-4" id="filterForm">
                <div>
                    <label class="block text-sm font-medium text-gray-700 mb-2">Tanggal Mulai</label>
                    <input type="date" name="start_date" required value="{{ request('start_date', $customStartDate ?? $startDate->format('Y-m-d')) }}"
                ...
                <div>
                    ...
                    <input type="date" name="end_date" required value="{{ request('end_date', $customEndDate ?? $endDate->format('Y-m-d')) }}"
                           class="w-full px-4 py-2.5 border border-gray-300 rounded-lg focus:ring-2 focus:ring-green-500 focus:border-green-500 transition">
                </div>
            ...
            <div class="p-6 border-b">
                <div class="flex justify-between items-start">
                    <div>
                        ...
                        <p class="text-sm text-gray-600 mt-1">Menampilkan {{ $orderDetails->count() }} dari {{ $orderDetails->total() }} transaksi</p>
                    </div>
                    ...
                    <tbody>
                        @forelse($orderDetails as $index => $order)
                            <tr class="border-t border-gray-100 hover:bg-gray-50 transition duration-200">
                                <td class="px-4 py-3 text-gray-600">{{ ($orderDetails->currentPage() - 1) * $orderDetails->perPage() + $index + 1 }}</td>
        ...
        <div class="grid grid-cols-1 lg:grid-cols-3 gap-6">
            ...
            <div class="bg-white p-6 rounded-xl shadow-md border border-gray-100 lg:col-span-2 hover:shadow-lg transition duration-300">
                <div class="mb-4">
                    ...
                    <p class="text-xs text-gray-500 mt-1">Periode: {{ $startDate->format('d M Y') }} - {{ $endDate->format('d M Y') }}</p>
                </div>
                <div class="h-80">
            ...
            <div class="bg-white p-6 rounded-xl shadow-md border border-gray-100 hover:shadow-lg transition duration-300">
                <div class="mb-4">
                    ...
                    <p class="text-xs text-gray-500 mt-1">Top 5 produk periode {{ $startDate->format('d M') }} - {{ $endDate->format('d M Y') }}</p>
                </div>
                <div class="space-y-3">
                    @forelse($topProducts as $index => $product)
                        <div class="flex items-center gap-3 p-3 rounded-lg border border-gray-200 hover:border-green-300 hover:shadow-sm transition duration-200 bg-white">
                            <!-- Ranking Number -->
